"""Provider backend for MeteoChile CAP feeds."""
...
class MeteoChileBackend(WarningBackend):
    """Fetch alerts from MeteoChile CAP feeds."""

    backend_id = 'meteochile'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a MeteoChile CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked MeteoChile CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _meteochile_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _meteochile_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a MeteoChile RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the MeteoChile RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/rss/' in url.lower() and url.lower().endswith('_cap.xml'):
            urls.append(url)
