"""Provider backend for Bahrain CAP feeds."""
...
class BahrainBackend(WarningBackend):
    """Fetch alerts from Bahrain CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Bahrain CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Bahrain CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _bahrain_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _bahrain_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Bahrain RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Bahrain RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/bh-meteo-' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
