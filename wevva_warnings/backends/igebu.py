"""Provider backend for IGBU Burundi CAP feeds."""
...
class IGEBUBackend(WarningBackend):
    """Fetch alerts from IGBU Burundi CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for an IGBU CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked IGBU CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _igebu_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _igebu_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from an IGBU RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the IGBU RSS feed.
    base_url : str
