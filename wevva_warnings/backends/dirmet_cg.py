"""Provider backend for Congo CAP feeds."""
...
class DirmetCGBackend(WarningBackend):
    """Fetch alerts from Congo CAP feeds."""

    backend_id = 'dirmet_cg'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Congo CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Congo CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _dirmet_cg_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _dirmet_cg_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Congo RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Congo RSS feed.
    base_url : str
