"""Provider backend for Finnish Meteorological Institute CAP feeds."""
...
class FMIBackend(WarningBackend):
    """Fetch alerts from Finnish Meteorological Institute CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for an FMI CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked FMI CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _fmi_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _fmi_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from an FMI RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the FMI RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and ('/cap/' in url.lower() or url.lower().endswith('.xml')):
            urls.append(url)
    return urls
