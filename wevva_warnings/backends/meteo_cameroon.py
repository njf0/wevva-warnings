"""Provider backend for Cameroon National Meteorology CAP feeds."""
...
class MeteoCameroonBackend(WarningBackend):
    """Fetch alerts from Cameroon National Meteorology CAP feeds."""

    backend_id = 'meteo_cameroon'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Cameroon CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Cameroon CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _meteo_cameroon_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _meteo_cameroon_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Cameroon RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Cameroon RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/cm-meteo-' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
