"""Provider backend for Macao SMG CAP feeds."""
...
class SMGBackend(WarningBackend):
    """Fetch alerts from Macao Meteorological and Geophysical Bureau CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for an SMG CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked SMG CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _smg_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _smg_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from an SMG RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the SMG RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and 'rss.smg.gov.mo/cap_' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
