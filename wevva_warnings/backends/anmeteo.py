"""Provider backend for Guinea CAP feeds."""
...
class ANMETEOBackend(WarningBackend):
    """Fetch alerts from Guinea CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Guinea CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Guinea CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _anmeteo_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _anmeteo_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Guinea RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Guinea RSS feed.
    base_url : str
