"""Provider backend for Ecuador INAMHI CAP feeds."""
...
class INAMHIBackend(WarningBackend):
    """Fetch alerts from Ecuador INAMHI CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for an Ecuador INAMHI CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Ecuador INAMHI CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _inamhi_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _inamhi_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from an Ecuador INAMHI RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Ecuador INAMHI RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/ec-inamhi-es/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
