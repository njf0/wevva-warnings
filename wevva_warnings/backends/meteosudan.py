"""Provider backend for Sudan CAP feeds."""
...
class MeteoSudanBackend(WarningBackend):
    """Fetch alerts from Sudan CAP feeds."""

    backend_id = 'meteosudan'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Sudan CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Sudan CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _meteosudan_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _meteosudan_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Sudan RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Sudan RSS feed.
    base_url : str
