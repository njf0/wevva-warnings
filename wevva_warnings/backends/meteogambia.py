"""Provider backend for Gambia CAP feeds."""
...
class MeteoGambiaBackend(WarningBackend):
    """Fetch alerts from Gambia CAP feeds."""

    backend_id = 'meteogambia'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Gambia CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Gambia CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _meteogambia_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _meteogambia_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Gambia RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Gambia RSS feed.
    base_url : str
