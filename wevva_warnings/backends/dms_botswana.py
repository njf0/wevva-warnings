"""Provider backend for Botswana Department of Meteorological Services CAP feeds."""
...
class DMSBotswanaBackend(WarningBackend):
    """Fetch alerts from Botswana Department of Meteorological Services CAP feeds."""

    backend_id = 'dms_botswana'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Botswana CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Botswana CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _dms_botswana_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _dms_botswana_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Botswana RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Botswana RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/bw-met-en/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
