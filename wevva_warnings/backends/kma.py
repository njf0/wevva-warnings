"""Provider backend for KMA CAP feeds."""
...
class KMABackend(WarningBackend):
    """Fetch alerts from Korea Meteorological Administration CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a KMA CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked KMA CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _kma_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _kma_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a KMA RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the KMA RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/w/repositary/xml/wrn/xml/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
