"""Provider backend for AEMET CAP feeds."""
...
class AEMETBackend(WarningBackend):
    """Fetch alerts from AEMET CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for an AEMET CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked AEMET CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _aemet_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _aemet_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from an AEMET RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the AEMET RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        title = (child_text(item, 'title') or '').lower()
        link = absolute_url(base_url, child_text(item, 'link'))
        if link is None:
            continue
        ...
        if lowered.endswith('.tar.gz'):
            continue
        if 'estado completo' in title:
            continue
        if lowered.endswith('.xml') and '/cap/' in lowered:
            urls.append(link)
