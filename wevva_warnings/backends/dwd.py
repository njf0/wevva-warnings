"""Provider backend for Deutscher Wetterdienst CAP feeds."""
...
class DWDBackend(WarningBackend):
    """Fetch alerts from Deutscher Wetterdienst CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a DWD CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked DWD CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _dwd_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _dwd_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a DWD RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the DWD RSS feed.
    base_url : str
