"""Provider backend for Trinidad and Tobago CAP feeds."""
...
class TTMSBackend(WarningBackend):
    """Fetch alerts from Trinidad and Tobago CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a TTMS CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked TTMS CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _ttms_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _ttms_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a TTMS RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the TTMS RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/public/api/feed/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
