"""Provider backend for Mongolia NAMEM CAP feeds."""
...
class NAMEMBackend(WarningBackend):
    """Fetch alerts from Mongolia NAMEM CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a NAMEM CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked NAMEM CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _namem_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _namem_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a NAMEM RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the NAMEM RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/mn-namem-en/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
