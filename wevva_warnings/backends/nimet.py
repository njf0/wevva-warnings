"""Provider backend for Nigeria NiMet CAP feeds."""
...
class NiMetBackend(WarningBackend):
    """Fetch alerts from Nigeria NiMet CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a NiMet CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked NiMet CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _nimet_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _nimet_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a NiMet RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the NiMet RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/ng-nimet-en/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
