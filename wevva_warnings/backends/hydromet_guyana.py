"""Provider backend for HydroMet Guyana CAP feeds."""
...
class HydroMetGuyanaBackend(WarningBackend):
    """Fetch alerts from HydroMet Guyana CAP feeds."""

    backend_id = 'hydromet_guyana'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a HydroMet Guyana CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked HydroMet Guyana CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _hydromet_guyana_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _hydromet_guyana_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a HydroMet Guyana RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the HydroMet Guyana RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/gy-hms-en/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
