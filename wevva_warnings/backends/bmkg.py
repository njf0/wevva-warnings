"""Provider backend for BMKG CAP feeds."""
...
class BMKGBackend(WarningBackend):
    """Fetch alerts from BMKG nowcast CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a BMKG CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked BMKG CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _bmkg_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _bmkg_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a BMKG RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the BMKG RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and url.lower().endswith('_alert.xml'):
            urls.append(url)
    return urls
