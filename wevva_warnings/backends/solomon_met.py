"""Provider backend for Solomon Islands CAP feeds."""
...
class SolomonMetBackend(WarningBackend):
    """Fetch alerts from Solomon Islands CAP feeds."""

    backend_id = 'solomon_met'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Solomon Islands CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Solomon Islands CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _solomon_met_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _solomon_met_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Solomon Islands RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Solomon Islands RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/sb-met-en/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
