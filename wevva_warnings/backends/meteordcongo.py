"""Provider backend for DR Congo CAP feeds."""
...
class MeteoRDCongoBackend(WarningBackend):
    """Fetch alerts from DR Congo CAP feeds."""

    backend_id = 'meteordcongo'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a DR Congo CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked DR Congo CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _meteordcongo_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _meteordcongo_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a DR Congo RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the DR Congo RSS feed.
    base_url : str
