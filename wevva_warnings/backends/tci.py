"""Provider backend for Turks and Caicos CAP feeds."""
...
class TCIBackend(WarningBackend):
    """Fetch alerts from Turks and Caicos CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Turks and Caicos CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Turks and Caicos CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _tci_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _tci_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Turks and Caicos RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Turks and Caicos RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/tc-gov-' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
