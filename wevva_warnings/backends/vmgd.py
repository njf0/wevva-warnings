"""Provider backend for Vanuatu CAP feeds."""
...
class VMGDBackend(WarningBackend):
    """Fetch alerts from Vanuatu Meteorology and Geo-Hazards CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a VMGD CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked VMGD CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _vmgd_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _vmgd_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a VMGD RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the VMGD RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/vu-vmgd-en/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
