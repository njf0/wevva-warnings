"""Provider backend for Maldives CAP feeds."""
...
class MMSBackend(WarningBackend):
    """Fetch alerts from Maldives CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for an MMS CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked MMS CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _mms_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _mms_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from an MMS RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the MMS RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/rss/alerts/' in url.lower():
