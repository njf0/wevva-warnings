"""Provider backend for Norwegian Water Resources and Energy Directorate feeds."""
...
class NVEBackend(WarningBackend):
    """Fetch alerts from NVE CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for an NVE CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked NVE CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _nve_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _nve_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from an NVE RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the NVE RSS feed.
    base_url : str
