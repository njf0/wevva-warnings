"""Provider backend for Argentina SMN CAP feeds."""
...
class SMNBackend(WarningBackend):
    """Fetch alerts from Servicio Meteorologico Nacional CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for an SMN CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked SMN CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _smn_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _smn_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from an SMN RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the SMN RSS feed.
    base_url : str
