"""Provider backend for Saint Lucia Meteorological Service CAP feeds."""
...
class SaintLuciaBackend(WarningBackend):
    """Fetch alerts from Saint Lucia CAP feeds."""

    backend_id = 'saint_lucia'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Saint Lucia CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Saint Lucia CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _saint_lucia_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _saint_lucia_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Saint Lucia RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Saint Lucia RSS feed.
    base_url : str
