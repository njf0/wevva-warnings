"""Provider backend for Dominican Republic INDOMET CAP feeds."""
...
class INDOMETBackend(WarningBackend):
    """Fetch alerts from INDOMET CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for an INDOMET CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked INDOMET CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _indomet_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _indomet_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from an INDOMET RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the INDOMET RSS feed.
    base_url : str
