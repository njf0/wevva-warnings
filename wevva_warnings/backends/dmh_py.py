"""Provider backend for Paraguay DMH CAP feeds."""
...
class DMHParaguayBackend(WarningBackend):
    """Fetch alerts from Paraguay DMH CAP feeds."""
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Paraguay CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Paraguay CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _dmh_py_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _dmh_py_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Paraguay RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Paraguay RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/py-dmh-es/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
