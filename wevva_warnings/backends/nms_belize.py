"""Provider backend for Belize National Meteorological Service CAP feeds."""
...
class NMSBelizeBackend(WarningBackend):
    """Fetch alerts from Belize National Meteorological Service CAP feeds."""

    backend_id = 'nms_belize'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Belize CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Belize CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _nms_belize_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _nms_belize_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Belize RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Belize RSS feed.
    base_url : str
    ...
    for item in root.iter():
        ...
        if url and '/bz-nms-en/' in url.lower() and url.lower().endswith('.xml'):
            urls.append(url)
