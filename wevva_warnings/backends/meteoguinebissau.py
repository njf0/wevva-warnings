"""Provider backend for Guinea-Bissau CAP feeds."""
...
class MeteoGuineaBissauBackend(WarningBackend):
    """Fetch alerts from Guinea-Bissau CAP feeds."""

    backend_id = 'meteoguinebissau'
    ...
    def fetch_alerts(
        ...
        *,
        ...
        """Fetch alerts for a Guinea-Bissau CAP source.
        ...
        -------
        list[Alert]
            Alerts parsed from linked Guinea-Bissau CAP documents.
        ...
        return fetch_cap_documents(
            source,
            _meteoguinebissau_alert_urls(root, base_url=source.url),
            preferred_lang=lang or source.lang,
            debug=debug,
        )


def _meteoguinebissau_alert_urls(root: ElementTree.Element, *, base_url: str) -> list[str]:
    """Return candidate CAP URLs from a Guinea-Bissau RSS feed.

    Parameters
    ----------
    root : ElementTree.Element
        Root XML element of the Guinea-Bissau RSS feed.
    base_url : str
